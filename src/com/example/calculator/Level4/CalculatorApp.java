...
public class CalculatorApp {
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            System.out.println("1. 두 수의 계산 | 2. 계산식 입력하기 | 3. 이전 계산 결과 확인 | 4. 삭제하기 | 5. 입력한 값보다 큰 값 확인 | 6. 종료하기(exit 입력 가능)");
            ...
            switch (command) {
                ...
                case "2" ->{
                    ...
                    System.out.println("계산식을 띄어쓰기와 함께 입력해주세요");
                    String calString = scanner.nextLine();

                    String[] expressionStr = divideExpression(calString);

                    double result = 0;
                    try{
                        result = arithmeticCalculator.ExpOperation(expressionStr);
                        arithmeticCalculator.setResultList(result);
                    }catch (ArithmeticException e){
    ...
    public static String[] divideExpression(String expression){
        ...
        for(String ep : expressionArr){
            try{
                double number = Double.parseDouble(ep);
                expressionStr += number + " ";
            } catch (NumberFormatException e) {
                if(ep.equals("(")) expressionStack.push("(");
                else if(ep.equals(")")){
                    while (!expressionStack.peek().equals("(")) expressionStr += expressionStack.pop() + " ";
                    expressionStack.pop();
                }else{
                    OperatorPriority priority = OperatorPriority.findPriority(ep);
                    while (!expressionStack.isEmpty()) {
                        String expInStack = expressionStack.peek();
                        if (priority.getPriority() <= OperatorPriority.findPriority(expInStack).getPriority())
                            expressionStr += expressionStack.pop() + " ";
                        else break;
                    }
                    expressionStack.push(ep);
                }
            }
        }

        while(!expressionStack.isEmpty()) expressionStr+= expressionStack.pop() + " ";
